...
Given paired data $\left( x_{i},y_{i} \right)$ for $i = 1,\ldots,n$,
`SeBR` implements efficient and fully Bayesian inference for
*semiparametric regression models* that incorporate (1) an unknown data
transformation

$$g\left( y_{i} \right) = z_{i}$$

and (2) a useful parametric regression model

$$z_{i} = f_{\theta}\left( x_{i} \right) + \sigma\epsilon_{i}$$

with unknown parameters $\theta$ and independent errors $\epsilon_{i}$.
