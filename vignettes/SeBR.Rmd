...
Unlike the Box-Cox version, `sbgp` respects the support of the data $\mathcal{Y} = [0,1]$, captures the trend, and provides narrower intervals (average widths are `r round(width, 3)` compared to `r round(width_bc,3)`) with similar coverage (`r emp_cov` for `sbgp` and `r emp_cov_bc` for Box-Cox). 
