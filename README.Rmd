---
title: "SeBR: Semiparametric Bayesian Regression"
author: "Dan Kowal"
date: "June 2023"
output: pdf_document  
...
*semiparametric regression models* that incorporate (1) an unknown
data transformation and (2) a useful parametric regression model. For paired data $\{x_i, y_i\}_{i=1}^n$ with $x_i \in \mathbb{R}^p$ and $y \in \mathcal{Y} \subseteq \mathbb{R}$, consider the following class of models:
\begin{align}
    \label{trans}
    g(y_i) &= z_i\\
    \label{mod}
...
**Examples.** We focus on the following important special cases of \eqref{mod}:
...
\end{equation}
The transformation \eqref{trans} broadens the applicability of this useful class of models, including for positive or compactly-supported data (see below), while \eqref{mod} corresponds to $P_{Z \mid \theta, X=x} = N(x'\theta, \sigma_\epsilon^2)$. 

2. The **quantile regression model** replaces the Gaussian assumption in \eqref{lm} with an *asymmetric Laplace* distribution (ALD) to target the $\tau$th quantile of $z$ at $x$, or equivalently, the $g^{-1}(\tau)$th quantile of $y$ at $x$. The ALD is quite often a very poor model for real data, especially when $\tau$ is near zero or one. The transformation \eqref{trans} offers a pathway to significantly improve the model adequacy, while still targeting the desired quantile of the data. 

3. The **Gaussian process (GP) model** generalizes \eqref{lm} to include a nonparametric regression function,
\begin{equation} \label{GP}
z_i = f_\theta(x_i) + \epsilon_i, \quad  \epsilon_i \stackrel{iid}{\sim} N(0, \sigma_\epsilon^2)
\end{equation}
where $f_\theta$ is a GP and $\theta$ parameterizes the mean and covariance functions. Although GPs offer substantial flexibility for the regression function $f_\theta$, the model \eqref{GP} may be inadequate when the $y$ has irregular marginal features or a restricted domain (e.g., positive or compact). 


**Challenges:** The goal is to provide fully Bayesian posterior inference for the unknowns $(g, \theta)$ and posterior predictive inference for future/unobserved data $\tilde y(x)$. We seek a model and algorithm that provide both (i) flexible modeling of $g$ and 
...
# Using \texttt{SeBR}
The \texttt{R} package \texttt{SeBR} is installed and loaded as follows:
...
The main functions in \texttt{SeBR} are:

- \texttt{sblm}: Monte Carlo sampling for posterior and predictive inference with the *semiparametric Bayesian linear model* \eqref{trans}--\eqref{lm};

- \texttt{sbsm}: Monte Carlo sampling for posterior and predictive inference with the *semiparametric Bayesian spline model*, which replaces \eqref{lm} with a spline for nonlinear modeling of $x \in \mathbb{R}$;

- \texttt{sbqr}: blocked Gibbs sampling for posterior and predictive inference with the *semiparametric Bayesian quantile regression*, which replaces the Gaussian distribution in \eqref{lm} with an ALD to target the $\tau$th quantile of the data; and

- \texttt{sbgp}: Monte Carlo sampling for predictive inference with the *semiparametric Bayesian Gaussian process model* \eqref{trans}--\eqref{mod} and \eqref{GP}. 


Each function returns a point estimate of $\theta$ (\texttt{coefficients}), posterior samples of the transformation $g$ (\texttt{post\_g}), and posterior predictive samples of $\tilde y(x)$ at some specified testing points $X_{test}$ (\texttt{post\_ytilde}), as well as other function-specific quantities (e.g., posterior draws of $\theta$, \texttt{post\_theta}).

**Note:** The package also includes Box-Cox variants of these functions, i.e., restricting $g$ to the (signed) Box-Cox parametric family $g(t; \lambda) = \{\mbox{sign}(t) \vert t \vert^\lambda - 1\}/\lambda$ with known or unknown $\lambda$. The parametric transformation is less flexible, especially for irregular marginals or restricted domains, and requires MCMC sampling. These functions (e.g., \texttt{blm\_bc}, etc.) are primarily for benchmarking. 

# Semiparametric Bayesian linear models with \texttt{sblm}

We simulate data from \eqref{trans}--\eqref{mod}:
...
\texttt{sblm} quickly produces Monte Carlo samples of $(\theta, g, \tilde y(X_{test}))$ under the semiparametric Bayesian linear model \eqref{trans}--\eqref{lm}:
...
First, we check for model adequacy using posterior predictive diagnostics. Specifically, we compute the empirical CDF on both \texttt{y\_test} and each simulated testing predictive dataset from \texttt{post\_ytilde}. 
...
**Remark:** Posterior predictive diagnostics do not require training/testing splits and are typically performed in-sample. If \texttt{X\_test} is left unspecified in \texttt{sblm}, then the posterior predictive draws are given at \texttt{X} and can be compared to \texttt{y}. Naturally, the out-of-sample diagnostics above offer a more rigorous check. 

Next, we evaluate the predictive ability on the testing dataset by computing and plotting the out-of-sample prediction intervals at \texttt{X\_test} and comparing them to \texttt{y\_test}:
...
Finally, we summarize the posterior inference for the transformation $g$ and the regression coefficients $\theta$ and compare to the ground truth values:
...
The posterior distribution of $g$ accurately matches the true transformation. The regression coefficients are also recovered.

**Remark:** The location-scale of the data-generating process and model \eqref{trans}--\eqref{lm} may not match exactly. Thus, we use correlations to compare the regression coefficients $\theta$ (while omitting the intercept) and apply location-scale shifts of the transformations $g$ to ensure comparability. This is only a byproduct of the simulated data setting and does not matter for real data analysis. 

**Note:** Try repeating this exercise with \texttt{blm\_bc} in place of \texttt{sblm}. The Box-Cox transformation cannot recover the transformation accurately, the model diagnostics are alarming, and the predictions deteriorate substantially.


# Semiparametric Bayesian quantile regression with \texttt{sbqr}

We now consider Bayesian quantile regression, which specifies a linear model for \eqref{mod} with ALD errors. First, we simulate data from a heteroskedastic linear model. Heteroskedasticity often produces conclusions that differ from traditional mean regression. Here, we do *not* include a transformation, so the data-generating process does not implicitly favor our approach over traditional Bayesian quantile regression (i.e., with $g(t) = t$ the identity). 
...
Now, we fit two Bayesian quantile regression models: the traditional version without a transformation (\texttt{bqr}) and the proposed alternative via \eqref{trans}--\eqref{mod} (\texttt{sbqr}). We target the $\tau = 0.05$ quantile.
...
For both model fits, we evaluate the same posterior predictive diagnostics as before. Specifically, we compute the empirical CDF on both \texttt{y\_test} and each simulated testing predictive dataset from \texttt{post\_ytilde}. 
...
Finally, we can asses the quantile estimates on the testing data. First, consider \texttt{bqr}:
...
Recall that these are *quantile* regression models at $\tau$, so we expect them to be asymmetric about \texttt{y\_test}. 
...
Repeat this evaluation for \texttt{sbqr}:
...
Now the out-of-sample empirical quantile is `r emp_quant_sbqr` and the 90\% prediction interval coverage is `r emp_cov_sbqr`. \texttt{sbqr} is better calibrated to $\tau$, while both methods are slightly overconservative in the prediction interval coverage. However, \texttt{sbqr} produce significantly smaller prediction intervals while maintaining this conservative coverage, and thus provides more powerful and precise inference. 
...
# Semiparametric Bayesian Gaussian processes with \texttt{sbgp}
...
Unlike the Box-Cox version, \texttt{sbgp} respects the support of the data $\mathcal{Y} = [0,1]$, captures the trend, and provides much narrower intervals (average widths are `r round(width, 3)` compared to `r round(width_bc,3)`) with about the same coverage (`r emp_cov` for \texttt{sbgp} and `r emp_cov_bc` for Box-Cox). 

Despite the significant complexities in the data, \texttt{sbgp} performs quite well out-of-the-box: 
...
**Note:** \texttt{sbgp} also applies for $x \in \mathbb{R}^p$ with $p >1$, such as spatial or spatio-temporal data. Such cases may require more careful consideration of the mean and covariance functions: the default mean function is a linear regression with the intercept only, while the default covariance function is an isotropic Matern function. However, many other options are available (inherited from the \texttt{GpGp} package).
